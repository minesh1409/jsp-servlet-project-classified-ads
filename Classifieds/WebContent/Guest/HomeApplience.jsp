...
String sql = "SELECT up.*,sc.*,u.*,l.*,c.* FROM user_product up, subcategory sc, user u,location l, category c  where up.subCategory_id=sc.id and up.user_id=u.id and u.location_id=l.id and sc.category_id =c.id and c.name='HomeApplience'";
...
<%}else{ %>
<form action="../PostController" method="post">
<p> <button type="submit"   class="btn btn-primary btn-xs" name="actionButton" value="approve"><i class="fa fa-edit"> </i> Approve
</p>
<input type="hidden" value="<%=resultSet.getString(1) %>" name="id"/>
</form>
<%} %>
